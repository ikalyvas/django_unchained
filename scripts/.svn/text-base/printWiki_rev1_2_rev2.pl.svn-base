...
sub run_the_path
{
    ...
    print("found : $found\n");
      ...
      if ($found > 0) {
         ...
         print "$Q\n";
         $sth2 = $dbh->prepare($Q); 
		 #$sth2 = $dbh->prepare("select trim(rev_id), trim(author), trim(bfr_lvn), trim(adaptation) from 'weblvnapp_wiki_entry' where (rev_id between '$r1' and '$r2')  and (branch_name_id= '$branch')");
         $sth2->execute();
	     for ($i=1; $i<=$found; $i++) {
            @data = $sth2->fetch();
	        foreach $item(@data) {
                             ...
                             #          1         2         3         4         5         6         7         8 
               print OUTFILE "---------------------------------------------------------------------------------\n";
               print OUTFILE "Adaptation:                                                                      \n";
	           print OUTFILE "@$item[0]\t  - @$item[1]\t - @$item[2]\n"; 
	           print OUTFILE "@$item[3]\n"; 
               print OUTFILE "---------------------------------------------------------------------------------\n";
               print OUTFILE "commit notes:                                                    \n"; 
               open (REVFILE, "<", "../my_media/svnhistory/@$item[0].txt") or die $!;
               my $iline;  
               my @revdata=<REVFILE>;
               foreach $iline(@revdata) {
                 print OUTFILE $iline; 
               }
               close REVFILE;
               print OUTFILE "--------------------------------------------------------------------------------\n";
