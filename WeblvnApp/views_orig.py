...
def uniqueLvn(request):
  ...
  #### code for input validation ####
    ...
    if len(arg_lvn)==0 and len(arg_branch)>0:
        ...
        cmd = "ls my_media/isu_compact/*|grep "+str(branch_set[0].branch_name)+'.'+'txt'+" |awk -F: '{print $1}'|awk -F/ '{print $NF}'|tail -1"
       
        output = []
        p = subprocess.Popen(cmd,shell=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
        output = p.communicate()[0].strip()
	  #  num=output.find('txt')
	  #	output = output[:num+3]#:as 3 is the length of 'txt' substring :#
        print 'length of %s is %s ' % (output,len(output))  
...
def do_svndiff(request):
    ...
    my_rev = request.GET.get('revision').strip()
    pattern = re.compile(r'/(\w+)(?=:(\d{5,6}))')
